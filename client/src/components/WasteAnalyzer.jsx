...
import { Leaf, Search, AlertCircle, TreePine, Recycle, ArrowRight, Loader2, Camera, X } from 'lucide-react';
...
const WasteAnalyzer = () => {
    ...
    const handleScan = (e) => {
        ...
        if (preview && fileInputRef.current?.files[0]) {
            analyze(fileInputRef.current.files[0]);
        } else if (input.trim()) {
            analyze(input);
        }
    };
            ...
            <div className="leaf-card p-10 text-center relative overflow-hidden group">
                ...
                <form onSubmit={handleScan} className="max-w-xl mx-auto space-y-6 relative z-10">
                    <div className="relative">
                        {preview ? (
                            <div className="relative w-full h-48 bg-slate-100 rounded-3xl overflow-hidden border-2 border-dashed border-[var(--accent-green)]/30 group/preview">
                                <img src={preview} alt="Preview" className="w-full h-full object-contain" />
                                <button 
                                    type="button"
                                    onClick={clearPreview}
                                    className="absolute top-4 right-4 p-2 bg-white/80 backdrop-blur-sm rounded-full text-rose-500 shadow-lg hover:bg-white transition-all"
                                >
                                    <X size={20} />
                                </button>
                                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover/preview:opacity-100 transition-opacity flex items-center justify-center">
                                    <p className="text-white font-black text-xs uppercase tracking-widest">Image Ready to Scan</p>
                                </div>
                            </div>
                        ) : (
                            <div className="relative">
                                <input 
                                    ...
                                    disabled={loading}
                                    placeholder="Type item or use camera →"
                                    className="earth-input pr-44 h-16 text-lg font-medium"
                                />
                                <div className="absolute right-3 top-2 bottom-2 flex gap-2">
                                    <button 
                                        ...
                                        disabled={loading}
                                        className="px-4 rounded-xl flex items-center justify-center bg-slate-100 text-[var(--accent-green)] hover:bg-slate-200 transition-all"
                                        title="Use Camera"
                                    >
                                        <Camera size={20} />
                                    </button>
                                    <button 
                                        ...
                                        className={`px-6 rounded-xl flex items-center gap-2 font-black text-xs tracking-widest uppercase transition-all ${
                                            loading || !input.trim()
                        ...
                        {preview && (
                            <button 
                                type="submit"
                                disabled={loading}
                                className="w-full h-16 mt-4 bg-[var(--accent-green)] text-white rounded-2xl font-black text-sm tracking-widest uppercase hover:bg-[var(--accent-leaf)] transition-all shadow-xl shadow-[var(--accent-green)]/20 flex items-center justify-center gap-3"
                            >
                                {loading ? (
